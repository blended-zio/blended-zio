...
object JNDISupport {
  ...
  def bind(ctxt: Context, name: String, ref: Reference): ZIO[Any, NamingException, Unit]   = ZIO.effect {
    ctxt.bind(name, ref)
  }.refineOrDie { case ne: NamingException => ne }

  def rebind(ctxt: Context, name: String, ref: Reference): ZIO[Any, NamingException, Unit] =
