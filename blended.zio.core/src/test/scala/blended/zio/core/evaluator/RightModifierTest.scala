...
object RightModifierTest extends DefaultRunnableSpec {
  ...
  override def spec: ZSpec[_root_.zio.test.environment.TestEnvironment, Any] = suite("The right modifier should")(
    testM("Simply take the n rightmost characters of a String")(checkM(for {
      s <- Gen.stringBounded(1, 100)(Gen.anyChar)
      n <- Gen.int(1, s.length)
    } yield (s, n)) { case (value, n) =>
      for {
        s <- RightModifier.modifier(value, n.toString)
      } yield assert(s)(equalTo(value.takeRight(n)))
    }),
    testM("Fail if the parameter is a negative number")(checkM(for {
      ...
      for {
        r <- RightModifier.modifier(value, n.toString).run
    ...
    testM("Fail if the parameter is not a number")(checkM(for {
      ...
      for {
        r <- RightModifier.modifier(value, n).run
      } yield assert(r)(fails(assertion("expect")() {
    ...
    testM("fail if called with a null value")(checkM(for {
      ...
      for {
        r <- RightModifier.modifier(null, n.toString).run
      } yield assert(r)(fails(assertion("expect")() {
        case me: ModifierException =>
